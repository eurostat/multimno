...
def export_map(df, output_path):
    ...
    census_map = SedonaPyDeck.create_choropleth_map(
        df=df, fill_color=fill_color)
...
if __name__ == "__main__":
    ...
    df.withColumn('NPRO', psf.trim('NPRO')).withColumn(
        'NCA', psf.lit('Euskadi')).createOrReplaceTempView("census")
