...
class SyntheticErrors(Component):
    ...
    def __init__(self, general_config_path: str, component_config_path: str):
        ...
        # static definition at the moment  #self.config.get(self.COMPONENT_ID, "optional_columns")
        ...
        self.date_bounds = [
            datetime.datetime.strptime(self.config.get(
                self.COMPONENT_ID, "starting_timestamp"), self.timestamp_format).date(),
            datetime.datetime.strptime(self.config.get(
                self.COMPONENT_ID, "ending_timestamp"), self.timestamp_format).date()
        ]
        self.sort_output = self.config.getboolean(
            self.COMPONENT_ID, "sort_output")

    def initalize_data_objects(self):

        self.error_generator_output_path = self.config.get(
            self.COMPONENT_ID, "error_generator_output_path")
        self.error_generator_input_path = self.config.get(
            self.COMPONENT_ID, "error_generator_input_path")
    ...
    def transform(self):
        ...
        # includes date partition column
        synth_df_raw = self.input_data_objects["SyntheticEvents"].df

        synth_df_raw = synth_df_raw.withColumn(
            ColNames.year, F.year(F.col(ColNames.timestamp)))
        synth_df_raw = synth_df_raw.withColumn(
            ColNames.month, F.month(F.col(ColNames.timestamp)))
        synth_df_raw = synth_df_raw.withColumn(
            ColNames.day, F.dayofmonth(F.col(ColNames.timestamp)))

        # Create a copy of original MSID column for final sorting and joining
        synth_df_raw = synth_df_raw.withColumn(
            "user_id_copy", F.col(ColNames.user_id))

        # Create event id column if missing
        if ColNames.event_id not in synth_df_raw.columns:
            windowSpec = Window.partitionBy(
                F.col(ColNames.user_id)).orderBy(F.col(ColNames.timestamp))
            synth_df_raw = synth_df_raw.withColumn(
                ColNames.event_id, F.row_number().over(windowSpec))

        # TODO optional column support
        synth_df = synth_df_raw[self.mandatory_columns +
                                ["user_id_copy", ColNames.event_id]]
        synth_df_w_nulls = self.generate_nulls_in_mandatory_fields(synth_df)
        synth_df_w_out_of_bounds_and_nulls = self.generate_out_of_bounds_dates(
            synth_df_w_nulls)
        synth_df_w_out_of_bounds_nulls_errors = self.generate_erroneous_type_values(
            synth_df_w_out_of_bounds_and_nulls)
    ...
    def generate_nulls_in_mandatory_fields(self, df: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
        ...
        result_df = df\
            .join(df_with_nulls, on=["user_id_copy", ColNames.event_id], how="leftanti")[[df.columns]]\
    ...
    def generate_out_of_bounds_dates(self, df: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
        ...
        events_span_in_months = pd.Timestamp(
            self.date_bounds[1]).month - pd.Timestamp(self.date_bounds[0]).month
        ...
        df_with_sample_column = df_not_null_dates.join(
            df_not_null_dates[[ColNames.event_id, ColNames.user_id]].sample(
                self.out_of_bounds_prob, seed=self.seed).withColumn("out_of_bounds", F.lit(True)),
        ...
        df_with_sample_column = df_with_sample_column.withColumn("timestamp",
                                                                 F.when(F.col("out_of_bounds"),
                                                                        F.add_months(
                                                                     F.col("timestamp"), F.col("months_to_add"))
    ...
    def generate_erroneous_type_values(self, df: pyspark.sql.DataFrame) -> pyspark.sql.DataFrame:
        ...
        df_with_sample_column = df_not_null.join(
            df_not_null[[ColNames.event_id, ColNames.user_id]].sample(
                self.error_prob, seed=self.seed).withColumn("mutate_to_error", F.lit(True)),
            on=[ColNames.user_id, ColNames.event_id],
            how="left"
        )
        ...
        for struct_schema in BronzeEventDataObject.SCHEMA:
            ...
            if column == ColNames.cell_id and col_dtype == StringType():
                random_string = ''.join(random.choice(
                    string.ascii_uppercase + string.digits) for _ in range(6)) + "_"
                to_value = F.concat(F.lit(random_string),
                                    (F.rand() * 100).cast("int"))
        ...
        result_df = df.join(df_with_sample_column, on=["user_id_copy", ColNames.event_id], how="leftanti")[df.columns]\
