...
class SyntheticEvents(Component):
    ...
    def __init__(self, general_config_path: str, component_config_path: str):
        ...
        if timestamp_generator_type == TimestampGeneratorType.EQUAL_GAPS:
            self.timestamp_generator_params = (timestamp_generator_type.value, self.starting_timestamp, self.ending_timestamp)
    ...
    def transform(self):
        ...
        records_df = records_df.withColumn(
            ColNames.timestamp, col(ColNames.timestamp).cast(StringType()))
        records_df = records_df.withColumn(
            ColNames.cell_id, col(ColNames.cell_id).cast(StringType()))
        records_df = records_df.withColumn(
    ...
    def generate_out_of_bounds_dates(self, df: DataFrame) -> DataFrame:
        ...
        events_span_in_months = pd.Timestamp(
            self.ending_timestamp).month - pd.Timestamp(self.starting_timestamp).month
