...
from tests.test_code.multimno.components.ingestion.inspire_grid_generation.aux_inspire_grid_generation import (
    generate_expected_inspire_grid_from_extent,
)
...
def test_inspire_grid_generation_extent(spark):
    """
    DESCRIPTION:
    This test verifies the INSPIRE grid generation component. It initialises the necessary configurations, sets up the
    input data, executes the grid generation process, and asserts that the output DataFrame matches the expected
    result
    ...
    2. Initialise the InspireGridGeneration component with the test configuration
    3. Execute the INSPIRE grid generation component.
    4. Read the output of the INSPIREGridGeneration component.
    5. Get the expected result of the execution.
    6. Assert equality of component output and expected output.
    """
    # Setup

    ## Init configs & paths
    component_config_path = f"{TEST_RESOURCES_PATH}/config/inspire_grid_generation/inspire_grid_generation_extent.ini"

    ## Init component class
    igg = InspireGridGeneration(TEST_GENERAL_CONFIG_PATH, component_config_path)

    # Expected
    expected_df = generate_expected_inspire_grid_from_extent(spark)
    ...
    output_grid_data_object = igg.output_data_objects[SilverGridDataObject.ID]
    output_grid_data_object.read()

    # assert read data == expected
    assert_sparkgeodataframe_equal(output_grid_data_object.df, expected_df)
